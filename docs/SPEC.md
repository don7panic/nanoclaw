...
│   ├── registered_groups.json     # Channel ID → folder mapping
...
Set the Discord bot token in `.env`:

```bash
DISCORD_BOT_TOKEN=your-bot-token
```

Enable **Message Content Intent** for the bot in the Discord Developer Portal, otherwise message content will be empty.

Optional (useful for invite links / documentation):
```bash
DISCORD_APP_ID=your-application-id
```

At startup, NanoClaw loads `.env` into `process.env` (existing environment variables take precedence).
...
| Discord Bot Token | .env / environment | Required for bot login |
